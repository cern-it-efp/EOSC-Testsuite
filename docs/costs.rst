...
There you must specify the price per hour for the different resources:
...
If a price is required for the cost calculation but the *costCalculation* section is not properly filled (For example, S3 Endpoint test was set
to True but *s3bucketPrice* was not set), no approximation will be given.

At the end of the run, the resulting approximated cost will be added to the results json file.
