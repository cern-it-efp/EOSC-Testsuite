# TODO: By default not enough space on the device: block_device is optional and default disk is 4GB for ubu18.
# Document requirements for launcher machine (PC/VM) on the docs
variable "public_ip" {
...
# TODO: with ansible (would require another docker image, for example "bastion-launcher", to avoid dependencies):
# -Allow root (needed?)
# -Install and start docker
# -Run test suite container in background (note this would save the user from running the tslauncher image, that would be transparent for him/her)
# -Connect ssh and exec into the container
