...
resource "cloudscale_server" "kubemaster" {
  ...
  image_slug = "centos-7"
  flavor_slug = "flex-2"
  ssh_keys = ["ssh-rsa  Generated by control.cloudscale.ch for ipeluaga@gmail.com"]
  ...
  provisioner "local-exec" {
    command = "echo ${self.public_ip} > ../master_ip && ./ssh_connect.sh --usr root --ip ${self.public_ip} --file aux/cluster_creator.sh --opts -m; mkdir -p ~/.kube & ./ssh_connect.sh --scp --src root@${self.public_ip}:~/.kube/config --dst ~/.kube/config"
  }
  provisioner "remote-exec" {
    inline = [
...
resource "cloudscale_server" "kubenode" {
  ...
  provisioner "local-exec" {
    command = "./ssh_connect.sh --usr root --ip ${self.public_ip} --file aux/cluster_creator.sh"
  }
  provisioner "local-exec" {
