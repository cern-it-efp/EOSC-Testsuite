...
resource "ibm_compute_vm_instance" "kubemaster" {
  ...
  provisioner "local-exec" {
    command = "echo ${self.ip_address} > ../master_ip && ./ssh_connect.sh --usr root --ip ${self.ip_address} --file aux/cluster_creator.sh --opts -m; mkdir -p ~/.kube & ./ssh_connect.sh --scp --src root@${self.ip_address}:~/.kube/config --dst ~/.kube/config"
  }
  provisioner "remote-exec" {
    inline = [
...
resource "ibm_compute_vm_instance" "kubenode" {
  ...
  provisioner "local-exec" {
    command = "./ssh_connect.sh --usr root --ip ${self.ip_address} --file aux/cluster_creator.sh"
  }
  provisioner "local-exec" {
